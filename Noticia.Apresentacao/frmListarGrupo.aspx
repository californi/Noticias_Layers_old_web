<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="frmListarGrupo.aspx.cs" Inherits="Noticia.Apresentacao.frmListarGrupo" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <fieldset>
        <legend>Filtrar Grupo</legend>
        <table border="0" cellpadding="0" cellspacing="0">
            <tr>
                <td style="padding-left: 10px; padding-right: 10px; height: 26px;">
                    <asp:Label ID="lblDescricao" runat="server" Text="Descrição: "></asp:Label></td>
                <td>
                    <asp:TextBox runat="server" MaxLength="49" Width="320px" ID="txtDescricao"></asp:TextBox>
                </td>
            </tr>
            <tr>
                <td>
    ...
    <asp:GridView runat="server" ID="grvGrupoTrabalho" Width="100%"
        AutoGenerateColumns="False" DataKeyNames="IdGrupoTrabalho"
        AllowSorting="True"
        EmptyDataText="Nenhum registro encontrado."
        OnRowCommand="grvGrupoTrabalho_RowCommand"
        OnRowDataBound="grvGrupoTrabalho_RowDataBound"
        Style="margin-right: 0px">
        <Columns>

            <asp:TemplateField HeaderText="Descrição" ConvertEmptyStringToNull="False" ControlStyle-Width="300">
                <ItemTemplate>
            ...
            <asp:TemplateField HeaderText="Ações">
                <ItemTemplate>
                    <asp:ImageButton runat="server" ID="ibtVisualizar" ImageUrl="~/Imagem/magnifier.png"
                        CommandArgument='<%# Eval("IdGrupoTrabalho") %>' CommandName="VISUALIZAR" CausesValidation="False" ToolTip="Visualizar" />
                    <asp:ImageButton runat="server" ID="ibtEditar" ImageUrl="~/Imagem/ico_edit_grid.gif"
                        CommandArgument='<%# Eval("IdGrupoTrabalho") %>' CommandName="EDITAR" CausesValidation="False" ToolTip="Editar" />
                    <asp:ImageButton runat="server" OnClientClick="if(confirm('Deseja realmente remover este usuário?')){return true;}else{return false;}" ID="ibtExcluir" ImageUrl="~/Imagem/ico_delete_grid.gif"
                        CommandArgument='<%# Eval("IdGrupoTrabalho") %>' CommandName="EXCLUIR" CausesValidation="False" ToolTip="Excluir" />
                </ItemTemplate>
            </asp:TemplateField>
